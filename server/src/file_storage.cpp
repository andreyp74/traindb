...
FileStorage::FileStorage(const std::string& dir) :
	dir_(dir)
{
	if (!fs::exists(dir_))
...
void FileStorage::put_data(data_map_type&& data)
{
	key_type key;
...
std::pair<key_type, std::string> FileStorage::write(data_map_type&& data) const
{
	if (data.empty())
...
data_map_type FileStorage::read(const std::string& file_name) const
{
